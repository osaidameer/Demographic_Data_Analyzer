...
def calculate_demographic_data(print_data=True):
    ...
    for id, name in enumerate(df['native-country'].value_counts().index.tolist()):
        percentage_list.append((name, round(len(df.loc[(df['native-country'] == name) & (df['salary'] == '>50K')]) / df['native-country'].value_counts()[id] * 100, 1)))

    highest_earning_country_tuple = max(percentage_list, key=lambda val: val[1])
    highest_earning_country = highest_earning_country_tuple[0]
    highest_earning_country_percentage = highest_earning_country_tuple[1]
